...
func (l *AnthropicLLM) Complete(ctx context.Context, prompt string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("anthropic llm: request failed: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", fmt.Errorf("anthropic llm: status %d: %v", resp.StatusCode, errBody)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", fmt.Errorf("anthropic llm: decode response: %w", err)
	}
	if len(result.Content) == 0 {
		return "", fmt.Errorf("anthropic llm: empty response")
	}
